...
export class AppService {
  ...
  public async logout(): Promise<boolean> {
    ...
    try {
      await this.api.post(url, {}, { withCredentials: true });
      return true;
    } catch (e) {
      return false;
    }
  ...
  public async fetchDocumentList(): Promise<any> {
    const url = Endpoint.GET_DOCUMENT_LIST;
    const { data } = await this.api.get(url, { withCredentials: true });
    return data;
  }
  ...
  public async createDocument(formData: any): Promise<any> {
    const url = Endpoint.GET_DOCUMENT_LIST;
    ...
    instance.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        const errorMessage: string =
          ...
          error.message;
        this.toastManager.quickShow(errorMessage);
        return Promise.reject(error);
      }
    );
  }
